...
class Network(nn.Module):
    def __init__(
        ...
        policy_net.append(layer_init(nn.Linear(last_layer_dim, caction_dim), gain=0.01))
    ...
    def get_distribution(self, state):
        ...
        clog_prob = self.policy_net(state)
        ...
        return Categorical(logits=masked_clogit)
...
class PPOAgent(object):
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            cdist = self.network.get_distribution(state)
            caction = cdist.sample()
            log_prob = cdist.log_prob(caction)
        return caction.cpu().numpy().flatten(), log_prob.cpu().numpy().flatten()
    ...
    def train(self):
        ...
        for _ in range(self.k_epoch):
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, True):
                new_cdist = self.network.get_distribution(
                        state=state[index]
                    )
                new_log_prob = new_cdist.log_prob(caction[index].squeeze()).unsqueeze(1)
