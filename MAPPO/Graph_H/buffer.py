...
class RolloutBuffer(object):
    def __init__(
        ...
        # 路径相关
        self.obs_feature = np.zeros((steps, num_env) + obs_features_shape, dtype=np.float32)
        self.global_cs_feature = np.zeros((steps, num_env) + global_features_shape, dtype=np.float32)
        ...
        self.next_obs_feature = np.zeros((steps, num_env) + obs_features_shape, dtype=np.float32)
        self.next_global_cs_feature = np.zeros((steps, num_env) + global_features_shape, dtype=np.float32)
    ...
    def rpush(self, reward, next_obs_feature, next_global_cs_feature, done, env_id):
    ...
    def rpush_last_state(self, obs_feature, global_cs_feature, action, action_mask, log_prob, env_id):
        rptr = self.rptr[env_id]
